...
public class UsersQueueExtension implements
    ...
    private final static Map<User.UserType, Queue<UserJson>> USERS_QUEUES = new ConcurrentHashMap<>();

    static {
        USERS_QUEUES.put(User.UserType.WITH_FRIENDS, new LinkedList<>(List.of(
                userJson("nick", "12345"),
                userJson("kyle", "12345"))
        ));

        USERS_QUEUES.put(User.UserType.INVITATION_SENT, new LinkedList<>(
                List.of(userJson("emma", "12345"),
                        userJson("emily", "12345"))
        ));

        USERS_QUEUES.put(User.UserType.INVITATION_RECEIVED, new LinkedList<>(
    ...
    public void beforeEach(ExtensionContext context) throws Exception {
        ...
        for (Parameter parameter : testParameters) {
            ...
            User.UserType userType = parameter.getAnnotation(User.class).userType();
            UserJson user = null;
            while (user == null) {
    ...
    public UserJson resolveParameter(ParameterContext parameterContext,
        ...
        Map<User.UserType, Map<String, UserJson>> users = context
    ...
    public void afterTestExecution(ExtensionContext context) throws Exception {
        ...
        Map<User.UserType, Map<String, UserJson>> users =
                context.getStore(USER_EXTENSION_NAMESPACE)
                        .get(testId, Map.class);
        // Добавляет все объекты UserJson из Map users обратно в соответствующую очередь в USERS_QUEUES
        for (Map.Entry<User.UserType, Map<String, UserJson>> entryUsers : users.entrySet()) {
